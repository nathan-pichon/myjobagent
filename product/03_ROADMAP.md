...
- [x] **3.5** **Dashboard temps réel** : la page (mode serveur) poll `/api/state` toutes les 2,5 s → bandeau live « chasse en cours » avec phase + compteurs (matches / offres vues), toast « ✦ nouvelles offres — Afficher » quand de nouvelles fiches arrivent, bouton « ▶ Lancer une chasse » dans le header. SQLite = bus temps réel (la boucle écrit, le serveur lit) ; `loop.run(on_progress=…)` publie l'avancement via le Scheduler. Dégradation gracieuse en mode `--static`. Testé (loop progress, /api/state, jsdom bandeau+toast).
